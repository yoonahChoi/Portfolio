...
public class BoardDaoSqls {
	public static final String SELECT_LIST = "SELECT board.id, name AS category, title, content, writer, reg_date, hits, likes "
			+ "FROM board JOIN category ON board.categoryId = category.id ORDER BY id DESC limit :start, :limit;";

	public static final String SELECT_LIST_BY_CATEGORY_ID = "SELECT board.id, name AS category, title, content, writer, reg_date, hits, likes "
			+ "FROM board JOIN category ON board.categoryId = category.id WHERE category.id = :categoryId ORDER BY id DESC limit :start, :limit;";

	public static final String SELECT_BOARD = "SELECT board.id, name as category, title, content, writer, reg_date, hits, likes "
